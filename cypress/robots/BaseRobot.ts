...
export abstract class BaseEyes {
  ...
  seesTextWtihDataTestId(domtestid: string, index:number, text: string){
    cy.get(`[data-testid=${domtestid}]`)
      .eq(index)
      .should('contain', text);
    return this;
  }
  ...
  containsTextWithClass(domClass: string, text: string){
    cy.get(`${domClass}`).should("contain",`${text}`)
    return this;
  }
...
export class BaseHands {
  ...
  ClickOnChildDomElement(dom: string, index:number){
    cy.get(`${dom}`)
    .eq(index)
    .click();
    return this;
  }
  ClickOnInvokedChildDomElement(dom: string, index:number){
    cy.get(`${dom}`)
    .eq(index)
  ...
  fixture(json: string, object:string, domlable:string, variable: string){
  cy.fixture(`${json}`).then((object)=>{
    cy.get(`${domlable}`).type(object.variable)
  })
}
